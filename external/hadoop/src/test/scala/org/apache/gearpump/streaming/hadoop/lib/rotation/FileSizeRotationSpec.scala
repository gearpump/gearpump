...
class FileSizeRotationSpec extends PropSpec with PropertyChecks with Matchers {
  ...
  val fileSizeGen = Gen.chooseNum[Int](1, 1000)

  property("FileSize rotation rotates on file size") {
    forAll(timestampGen, fileSizeGen) { (timestamp: TimeStamp, fileSize: Int) =>
      List(
        Rotation(fileSize B),
        Rotation(fileSize KB),
        Rotation(fileSize MB),
        Rotation(fileSize GB),
        Rotation(fileSize TB)).foreach {
        rotation =>
