...
class KafkaMessageIteratorSpec extends PropSpec with PropertyChecks with Matchers with MockitoSugar {
  ...
  property("KafkaMessageIterator should iterate MessageAndOffset calling hasNext and next") {
    forAll(messageGen, messageNumGen, topicAndPartitionGen) {
      (message: Message, num: Int, topicAndPartition: (String, Int)) =>
        ...
        val kafkaMsgIterator = new KafkaMessageIterator(consumer, topic, partition, getIterator)
        0.until(num).foreach { i =>
          kafkaMsgIterator.hasNext shouldBe true
          val kafkaMessage = kafkaMsgIterator.next
          kafkaMessage.offset shouldBe i.toLong
          kafkaMessage.key shouldBe None
        }
        kafkaMsgIterator.hasNext shouldBe false
    }
  }

  val startOffsetGen = Gen.choose[Long](1L, 1000L)
  property("KafkaMessageIterator setStartOffset should reset internal iterator") {
    forAll(topicAndPartitionGen, startOffsetGen) {
      (topicAndPartition: (String, Int), startOffset: Long) =>
        ...
        when(consumer.earliestOrLatestOffset(TopicAndPartition(topic, partition),
          OffsetRequest.EarliestTime, -1)).thenReturn(0)
        val kafkaMsgIterator = new KafkaMessageIterator(consumer, topic, partition, getIterator)
        kafkaMsgIterator.setStartOffset(startOffset)
        verify(getIterator).apply(startOffset)
    }
  }

  property("KafkaMessageIterator close should close consumer") {
