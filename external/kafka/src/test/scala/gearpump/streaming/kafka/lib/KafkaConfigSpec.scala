...
class KafkaConfigSpec extends PropSpec with Matchers with MockitoSugar {
  ...
  property("kafka conf should be set correctly") {
    ...
    assert(Try(config.getProducerBootstrapServers).isSuccess)
    assert(Try(config.getProducerAcks).isSuccess)
    assert(Try(config.getProducerBufferMemory).isSuccess)
    assert(Try(config.getProducerCompressionType).isSuccess)
    assert(Try(config.getProducerBatchSize).isSuccess)
    assert(Try(config.getProducerRetries).isSuccess)
