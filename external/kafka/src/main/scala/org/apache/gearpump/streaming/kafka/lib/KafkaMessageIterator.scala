...
object KafkaMessageIterator {
  def apply(host: String,
            ...
            soBufferSize: Int,
            fetchSize: Int,
            clientId: String): KafkaMessageIterator = {
    val consumer = new SimpleConsumer(host, port, soTimeout, soBufferSize, clientId)
    ...
    new KafkaMessageIterator(consumer, topic, partition, getIterator)
  }
}

class KafkaMessageIterator(consumer: SimpleConsumer,
                           topic: String,
                           partition: Int,
  ...
  def next(): KafkaMessage = {
    ...
    KafkaMessage(TopicAndPartition(topic, partition), offset, Option(message.key).map(Utils.readBytes), payload)
  }
