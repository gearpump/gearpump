...
//    IO.copyFile(core.base / sharedMessages, file(Build.servicesjs.baseDirectory) / sharedMessages)
  ...
  def copyJSArtifactsToOutput: Unit = {
    ...
    val in = file(s"${Build.servicesjs.base.getPath}/target/scala-2.11/gearpump-services-js-fastopt.js.map")
    val out = file(s"${Build.distDirectory}/target/pack/dashboard/gearpump-services-js-fastopt.js.map")
    val input = IO.read(in)
    val data = input.replaceAll("../../src","src")
    IO.write(out, data)
    IO.copyFile(
      file(s"${Build.servicesjs.base.getPath}/target/scala-2.11/gearpump-services-js-fastopt.js"),
      file(s"${Build.distDirectory}/target/pack/dashboard/gearpump-services-js-fastopt.js")
    )
    IO.copyDirectory(
      file(s"${Build.servicesjs.base.getPath}/src"),
      file(s"${Build.distDirectory}/target/pack/dashboard"),
      true
    )
    IO.copyDirectory(
      file(Build.servicesjs.base.getPath),
      file(Build.distDirectory) / "target" / "pack" / "dashboard",
      true
    )
