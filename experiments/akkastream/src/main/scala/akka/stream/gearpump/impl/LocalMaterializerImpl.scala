...
import akka.stream.impl.{FlowNameCounter, StreamSupervisor, ActorMaterializerImpl, ActorProcessorFactory, ActorPublisher, Balance, Broadcast, Concat, ExposedPublisher, FairMerge, FanIn, FanOut, FlexiMerge, FlexiRoute, SinkModule, SourceModule, UnfairMerge, VirtualProcessor}
import akka.stream.io.SslTls.TlsModule
import akka.stream.{Graph => AkkaGraph, ActorMaterializerSettings, ActorMaterializer, Attributes, InPort, MaterializationContext, Materializer, ModuleGraph, Optimizations, OutPort, Shape}
import io.gearpump.util
...
class LocalMaterializerImpl (
  ...
  override def materialize(graph: Graph[Module, Edge]): Map[Module, Any] = {
    ...
    materializedGraph.vertices.map{vertex =>
