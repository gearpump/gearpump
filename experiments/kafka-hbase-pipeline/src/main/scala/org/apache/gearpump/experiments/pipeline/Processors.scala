...
class CpuPersistor(taskContext : TaskContext, conf: UserConfig)
  ...
  override def onNext(msg: Message): Unit = {
    Try({
      val jsonData = msg.msg.asInstanceOf[String]
      val cpus = read[Array[Datum]](jsonData)
      LOG.info(s"received ${cpus.length}")
      cpus.foreach(cpu => {
        LOG.info(s"inserting ${msg.timestamp.toString} $family $column $cpu")
        insert(msg.timestamp.toString, family, column, cpu)
      })
    }).failed.foreach(LOG.error("bad message", _))
  }
...
class MemoryPersistor(taskContext : TaskContext, conf: UserConfig)
  ...
  override def onNext(msg: Message): Unit = {
    Try({
      val jsonData = msg.msg.asInstanceOf[String]
      val memories = read[Array[Datum]](jsonData)
      LOG.info(s"received ${memories.length}")
      memories.foreach(memory => {
        LOG.info(s"inserting ${msg.timestamp.toString} $family $column $memory")
        insert(msg.timestamp.toString, family, column, memory)
      })
    }).failed.foreach(LOG.error("bad message", _))
  }
...
trait HBaseConsumer {
  ...
  val hBaseSink = HBaseSink(table, hbaseConf)

  def userConf:UserConfig

  def insert(rowKey: String, family: String, column: String, value: String) = hBaseSink.insert(rowKey, family, column, value)
}
