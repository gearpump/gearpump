...
class ReplayableSourceTest extends Traverse[Array[Datum]] {
  ...
  val list = List(Message(data(0)),Message(data(1)),Message(data(2)))
  lazy val source = new TimeReplayableSource() {
    def startFromBeginning(): Unit = {}
    def setStartTime(startTime: TimeStamp): Unit = {}
    def pull(num: Int): List[Message] = list
    def close(): Unit = {}
  }
...
class PipeLineDSLSpec extends PropSpec with PropertyChecks with Matchers with BeforeAndAfter {
  ...
  property("Producer should generate Array[Datum]") {
    val context = ClientContext()
    val app = StreamApp("PipeLineDSL", context)
    val producer = app.source(new ReplayableSourceTest, 1, "time-replayable-producer")
