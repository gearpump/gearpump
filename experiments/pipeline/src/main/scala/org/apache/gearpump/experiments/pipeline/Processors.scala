...
class CpuProcessor(taskContext: TaskContext, conf: UserConfig)
  ...
  //TODO
  override def timeInterval = 200//conf.getInt(CPU_INTERVAL).getOrElse(DEFAULT_INTERVAL)
...
class MemoryProcessor(taskContext: TaskContext, conf: UserConfig)
  ...
  override def timeInterval = 200//conf.getInt(MEM_INTERVAL).getOrElse(DEFAULT_INTERVAL)
