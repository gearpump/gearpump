...
object PipeLine extends App with ArgumentsParser {
  ...
  override val options: Array[(String, CLIOption[Any])] = Array(
    ...
    "processors" -> CLIOption[Int]("<how many processor tasks>", required = false, defaultValue = Some(4)),
    "persistors" -> CLIOption[Int]("<how many persistor tasks>", required = false, defaultValue = Some(4))
  )
  ...
  Option(parse(args)).foreach(config => {
    ...
    def application(config: ParseResult) : AppDescription = {
      ...
      val kafka = TaskDescription(classOf[KafkaProducer].getName, 2)
      val cpuProcessor = TaskDescription(classOf[CpuProcessor].getName, processors)
      val cpuPersistor = TaskDescription(classOf[CpuPersistor].getName, persistors)
      val memoryProcessor = TaskDescription(classOf[MemoryProcessor].getName, processors)
      val memoryPersistor = TaskDescription(classOf[MemoryPersistor].getName, persistors)
