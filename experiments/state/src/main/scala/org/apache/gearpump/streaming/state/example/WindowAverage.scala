...
import org.apache.gearpump.streaming.state.api.WindowDescription
import org.apache.gearpump.streaming.state.example.processor.{WindowAverageProcessor, GenNumProcessor}
import org.apache.gearpump.streaming.state.impl.HadoopCheckpointStoreFactory
import org.apache.gearpump.streaming.state.internal.impl.KafkaCheckpointStoreFactory
...
object WindowAverage extends App with ArgumentsParser {
  ...
  def application(config: ParseResult) : StreamApplication = {
    ...
    val userConfig = UserConfig.empty
      .withValue[WindowDescription](StateConfig.WINDOW, WindowDescription(windowSize))
      .withLong(StateConfig.CHECKPOINT_INTERVAL_DEFAULT, 1000)
      .withString(StateConfig.CHECKPOINT_STORE_FACTORY, classOf[KafkaCheckpointStoreFactory].getName)
