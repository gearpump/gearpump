...
object StateUtil {
  val count = new StateOp[Long] {

    override def init: Long = 0L

    override def update(msg: Message, t: Long): Long = {
      t + 1L
    }

    override def serialize(t: Long): Array[Byte] = {
      Injection[Long, Array[Byte]](t)
    }

    override def deserialize(bytes: Array[Byte]): Long = {
      Injection.invert[Long, Array[Byte]](bytes)
        .getOrElse(throw new RuntimeException("deserialize failed"))
    }
  }
}
