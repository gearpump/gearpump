...
class WindowAverageProcessorSpec extends PropSpec with PropertyChecks with Matchers {
  property("WindowAverageProcessor should update state clock") {
    ...
    forAll(longGen, longGen, longGen) {
      (windowSize: Long, upstreamMinClock: Long, checkpointInterval: Long) =>
        ...
        val conf = UserConfig.empty
          ...
          .withValue[WindowDescription](StateConfig.WINDOW, WindowDescription(windowSize.millis, windowSize.millis))

        val windowAverage = new WindowAverageProcessor(taskContext, conf)
        windowAverage.onStart(StartTime(0L))
        windowAverage.onNext(Message("" + upstreamMinClock, 0L))
        windowAverage.onNext(Message("" + upstreamMinClock, checkpointInterval / 2))
        if (upstreamMinClock < checkpointInterval) {
          windowAverage.stateClock shouldBe Some(0L)
        } else {
