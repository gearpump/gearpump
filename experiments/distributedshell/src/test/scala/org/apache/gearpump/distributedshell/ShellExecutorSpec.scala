...
import org.apache.gearpump.experiments.cluster.ExecutorToAppMaster.{RegisterExecutor, ResponsesFromExecutor}
...
class ShellExecutorSpec extends WordSpec with Matchers {
  ...
  "ShellExecutor" should {
    "execute the shell command and return the result" in {
      ...
      assert(mockMaster.receiveN(1).head.asInstanceOf[ResponsesFromExecutor].equals(
        ResponsesFromExecutor(executorId, List(result))))
