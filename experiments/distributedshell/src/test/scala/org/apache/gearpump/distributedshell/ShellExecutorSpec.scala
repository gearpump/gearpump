...
import org.apache.gearpump.experiments.cluster.ExecutorToAppMaster.{RegisterExecutor, ResponseFromExecutor}
...
class ShellExecutorSpec extends WordSpec with Matchers {
  ...
  "ShellExecutor" should {
    "execute the shell command and return the result" in {
      ...
      assert(mockMaster.receiveN(1).head.asInstanceOf[ResponseFromExecutor].equals(
        ResponseFromExecutor(executorId, result)))
