  ...
  def translate(ops: OpChain)(implicit system: ActorSystem): TaskDescription = {
    ...
    ops.ops.head match {
      case op: MasterOp =>
        ...
        op match {
          case InMemoryCollectionSource(collection, parallism) =>
          ...
          case groupby@ GroupByOp(_, parallism) =>
          ...
          case merge: MergeOp =>
            TaskDescription(classOf[TransformTask[Object, Object]].getName, 1, taskConf = userConfig)
          case ProcessorOp(processor, parallism) =>
            TaskDescription(processor, parallism, null)
        }
      case op: SlaveOp =>
  ...
  private def toFunction(ops: List[Op]): SingleInputFunction[Object, Object] = {
    ...
    val totalFunction = ops.foldLeft(func) { (fun, op) =>
      ...
      val opFunction = op match {
        case flatmap: FlatMapOp[Object, Object] => new FlatMapFunction(flatmap.fun)
...
object OpTranslator {
  ...
  class FlatMapFunction[IN, OUT](fun: IN => TraversableOnce[OUT]) extends SingleInputFunction[IN, OUT] {
    def process(value: IN): TraversableOnce[OUT] = {
      fun(value)
    }
  }

  class ReduceFunction[T](fun: (T, T)=>T) extends SingleInputFunction[T, T] {
