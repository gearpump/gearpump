...
case class FlatMapOp[T, R](fun: (T) => TraversableOnce[R]) extends SlaveOp

case class ReduceOp[T](fun: (T, T) =>T) extends SlaveOp

trait MasterOp extends Op

case class MergeOp(source: Op, target: Op) extends MasterOp

case class GroupByOp[T, R](fun: T => R, parallism: Int) extends MasterOp

case class ProcessorOp(processor: String, parallism: Int) extends MasterOp

case class InMemoryCollectionSource[T](collection: List[T], parallism: Int) extends MasterOp
