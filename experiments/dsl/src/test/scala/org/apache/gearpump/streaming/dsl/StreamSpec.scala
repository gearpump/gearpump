...
class StreamSpec  extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  implicit var context =  ClientContext("master")
  ...
  it should "translate the DSL to a DAG" in {
    ...
    val dagTopology = appDescription.dag.mapVertex(_.taskClass).mapEdge(_.getClass.getName)
  ...
  private def getExpectedDagTopology: Graph[String, String] = {
    ...
    val expectedDagTopology = Graph[String, String](
      source ~ hash ~> group ~ colocation ~> merge ~ hash ~> join,
      source ~ hash ~> merge
    )
    expectedDagTopology
  }
