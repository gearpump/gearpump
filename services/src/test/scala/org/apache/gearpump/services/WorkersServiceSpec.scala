...
class WorkersServiceSpec extends FlatSpec with ScalatestRouteTest with WorkersService with Matchers with BeforeAndAfterEach {
  ...
  "WorkersService" should "return a json structure of worker list for GET request" in {
    implicit val customTimeout = RouteTestTimeout(15.seconds)
    (Get("/workers") ~> workersRoute).asInstanceOf[RouteResult] ~> check {
      //check the type
      read[WorkerList](response.entity.asString)
    }
  }
