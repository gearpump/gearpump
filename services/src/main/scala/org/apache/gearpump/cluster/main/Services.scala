...
object Services extends App with ArgumentsParser {
  ...
  override val options: Array[(String, CLIOption[Any])] = Array(
    ...
    "host" -> CLIOption[String]("host", required = false),
    "httpPort" -> CLIOption[Int]("httpPort", required = false),
    "wsPort" -> CLIOption[Int]("wsPort", required = false)
  )
  ...
  def start(): Unit = {
    Option(parse(args)).map(options => {
      Option(options.getString("master")).map(masterList => {
        ...
        val host = getOption("host", options)
        val httpPort = getOption("httpPort", options)
        val wsPort = getOption("wsPort", options)
        val config = Option(Seq(
          getProp(host, "gearpump.services.host=" + '"' + "%s" + '"'),
          getProp(host, "akka.remote.netty.tcp.hostname=" + '"' + "%s" + '"'),
          getProp(httpPort, "gearpump.services.http=%s"),
          getProp(wsPort, "gearpump.services.ws=%s")
        ).flatten.toArray.mkString("\n")).map(configString => {
          LOG.info(s"configString=$configString")
          ConfigFactory.parseString(configString).withFallback(system.settings.config)
        }).getOrElse(system.settings.config)
