...
class StreamingKmeansSpec extends PropSpec with PropertyChecks with Matchers with BeforeAndAfter with MasterHarness {
  before {
    startActorSystem()
  }
  ...
  property("StreamingKmeansExample should succeed to submit application with required arguments") {
    ...
    forAll(args) { (requiredArgs: Array[String], optionalArgs: Array[String]) =>
      ...
      Future {StreamingKmeansExample.main(masterConfig, args)}
