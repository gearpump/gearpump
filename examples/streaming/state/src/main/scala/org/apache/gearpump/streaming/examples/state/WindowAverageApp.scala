...
import org.apache.gearpump.streaming.state.system.impl.{PersistentStateConfig, WindowConfig}
...
object WindowAverageApp extends AkkaApp with ArgumentsParser {
  ...
  def application(config: ParseResult)(implicit system: ActorSystem) : StreamApplication = {
    ...
    val checkpointStoreFactory = new HadoopCheckpointStoreFactory("MessageCount", new Configuration, Rotation(1 GB))
