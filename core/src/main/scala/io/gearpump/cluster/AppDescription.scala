 ...
 *                      new applications. In most cases, you wouldnot need to change it. If you do
 *                      need to change it, use ClusterConfigSource(filePath) to construct the
 *                      object, while filePath points to the .conf file.
 */
 ...
 * @param masterProxy The proxy to master actor, it will bridge the messages between appmaster
 *                    and master
 * @param registerData The AppMaster are required to register this data back to Master by
 *                     RegisterAppMaster
 */
 ...
 *
 * @param classPath When a worker create a executor, the parent worker's classpath will be
 *                  automatically inherited, the application jar will also be added to runtime
 ...
 *                           It will use io.gearpump.util.Constants.GEARPUMP_CUSTOM_CONFIG_FILE
