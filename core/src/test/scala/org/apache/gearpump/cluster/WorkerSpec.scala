...
class WorkerSpec(_system: ActorSystem) extends TestKit(_system) with ImplicitSender
  ...
  "Worker" should {
    "update its remaining resource when launching and shutting down executors" in {
      ...
      val executionContext = ExecutorContext(Util.getCurrentClassPath, workerSystem.settings.config.getString(Constants.GEARPUMP_APPMASTER_ARGS).split(" "), classOf[ActorSystemBooter].getName, Array(executorName, reportBack))
