...
import gearpump.cluster.MasterToAppMaster.AppMasterRegistered
import gearpump.cluster.TestUtil
import gearpump.cluster.appmaster.MasterConnectionKeeper.MasterConnectionStatus._
import gearpump.cluster.appmaster.MasterConnectionKeeperSpec.ConnectionKeeperTestEnv
import gearpump.cluster.master.MasterProxy
import gearpump.cluster.master.MasterProxy.WatchMaster
...
class MasterConnectionKeeperSpec  extends FlatSpec with Matchers with BeforeAndAfterAll {
  ...
  it should "re-register the appmaster when master is restarted" in {
    import gearpump.cluster.master.MasterProxy.MasterRestarted
  ...
  it should "notify listener and then shutdown itself when master is dead" in {
    ...
    //master is dead
    master.send(keeper, MasterProxy.MasterStopped)
