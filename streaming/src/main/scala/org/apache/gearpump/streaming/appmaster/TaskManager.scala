...
import org.apache.gearpump.streaming.{TaskScheduler, DAG, TaskSchedulerImpl}
...
private[appmaster] class TaskManager(
  ...
  when (Uninitialized) {
    case Event(DagInit(dag), _) =>
      ...
      val resourceRequests = taskScheduler.dagUpdated(dag)
  ...
  private def startTasksAndHandleExecutorLoss: StateFunction = {
    ...
    case Event(ExecutorStopped(executorId), state@ TaskRegistrationState( register)) =>
      val newResourceRequests = taskScheduler.resourceFailed(executorId)
