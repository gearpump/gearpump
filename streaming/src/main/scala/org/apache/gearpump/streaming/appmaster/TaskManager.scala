...
import org.apache.gearpump.streaming.AppMasterToExecutor.{TaskRejected, TaskChanged, ChangeTasks, LaunchTasks, StartClock}
...
private[appmaster] class TaskManager(
  ...
  private val startTasks: StateFunction = {
    case Event(executor: ExecutorStarted, state) =>
      ...
      LOG.info(s"Going to schedule ${taskIds} tasks")
  ...
  when(DynamicDAG)(onMessageLoss orElse startTasks orElse onClockEvent)
  ...
  onTransition {
    case _ -> ApplicationReady =>
      ...
      dagManager ! DAGScheduled(newDagVersion)
